...
-- 이 경우에는 실제 저장되어있는 데이터들이 누락되어 출력되기 때문에
-- LEFT JOIN을 사용해서 조회
SELECT SC.s_num, ST.st_name, SC.s_kor, SC.s_eng, SC.s_math
FROM tbl_score -- 조회하고자 하는 주(main) 데이터
    LEFT JOIN tbl_student ST-- 연결해서 보고자 하는 보조(sub, detail)데이터
        ON SC.s_num = ST.st_num; -- 연계할 조건 설정
        
SELECT SC.s_num, ST.st_name, SC.s_kor, SC.s_eng, SC.s_math,
         SC.st_kor + SC.st_eng + SC.st_math AS 총점,
          ROUND( (SC.st_kor + SC.st_eng + SC.st_math)/3,0) AS 평균
FROM tbl_score SC
...
-- nVARCHAR2(3) : 유니코드 기준 3글자, 한글3글자 입력 가능
...
-- 매우 복잡한 SQL문을 매번 실행하는 것은 매우 비효율적(코딩 문제에서)이다.
...
-- 집계함수로 감싸지 않은 칼럼들은 
-- GROUP BY절 다음에 나열을 해 주어야 한다.

-- 학번, 학과코드로 묶어서 보여주는 데이터는
-- 의미없는 명령문이다.
-- ★ GROUP으로 묶어서 집계를 낼 때에는
--    어떤 칼럼들을 묶을 것인지에 대한 많은 고민을 해야한다.
