...
-- TABLE을 상대로 실행하면
-- TABLE에 저장된 데이터 개수만큼 반복되어서 실행이 된다.
-- 그래서 이런 문제점때문에 한개의 레코드만 가진 DUMMY TABLE을 준비해 두고
...
-- 실제 사용하고 있는 DBMS에 물리적인 테이블을 생성하지 않고 
-- 가상의 데이터를 만들어서 사용하고자 할 때 TABLE 구조와
-- 데이터를 생성하여 테스트 용도로 사용하는 명령군
...
UNION ALL SELECT s_num AS학번, to_char(s_kor,'999') AS 국어, to_char(s_eng,'999') AS 영어, to_char(s_math,'999') AS 수학 FROM tbl_score
UNION ALL SELECT '-----' AS 학번,'----' AS 국어,'----' AS 영어,'----' AS 수학 FROM DUAL
UNION ALL SELECT '총점' AS 학번, to_char(SUM(s_kor),'99999') AS 국어, 
                                to_char(SUM(s_eng),'99999') AS 영어, 
                                to_char(SUM(s_math),'99999') AS 수학 FROM tbl_score
UNION ALL SELECT '=====' AS 학번,'====' AS 국어,'====' AS 영어,'====' AS 수학 FROM DUAL;
...
-- 기은성, 남도예, 내세원, 갈한수, 바재호, 배재호

SELECT * FROM tbl_student
WHERE st_name IN ('기은성', '남동예', '내세원', '갈한수', '방채호','배재호');        
        
-- SUB QUERY
-- 1. WHERE 절에 사용하는 SUBQUERY
--    SUQ 중에 가장 많이 사용하는 것
--    중첩(서브)쿼리라고도 한다.
WITH tbl_temp AS
( 
...
SELECT * 
FROM tbl_student 
WHERE st_name IN (SELECT T.name FROM tbl_temp );


/*
        WHERE 절에 포함된 SELECT 실행 : SELECT name FROM tbl_temp
        '기은성', '남동예', '내세원','방채호','배재호' 시트를 생성하고
        WHERE ('기은성', '남동예', '내세원','방채호','배재호') 코드를 생성한다.
...
-- 단순한 JOIN을 이용해서 보여주기 어려운 통계, 집계 등을 사용할 때
...
-- 3. SELECT SUB
-- 스칼라서브 쿼리
-- 1, 2 쿼리와 달리 SUBQ에서는 절대 LIST를 출력하면 안 된다.
--    SUBQ 에서는 단 1줄의 Record만 결과로 나와야 한다.

-- 가. tbl_student 테이블을 for() 반복문으로 반복하기
-- 나. tbl_student의 st_num 칼럼값을 sub 쿼리에 전달
