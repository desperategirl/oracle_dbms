...
-- tbe_student 테이블에 데이터를 추가하고, 조회하기
...
SELECT st_num, st_name, st_addr, st_tel -- 별표 제외
FROM tbl_student ;

-- 3. tbl_student에서 이름, 학번, 전화번호, 주소 순서로 칼럼을 리스트에 보이면 좋겠다.
SELECT st_name, st_num, st_tel, st_addr 
FROM tbl_student ;

-- 4. 리스트를 볼 때 원래의 칼럼이름 대신에 별명을 사용하기
...
-- 데이터 리스트(row, recored)중에서 필용한 부분만 보고 싶을 때
...
-- SELECT 명령문을 사용할 때
-- 칼럼 리스트를 *로 사용하지 않고 필요한 칼럼만 나열해주는 것이
-- 많은 양의 데이터를 조회할 때는 속도 면에서 다소 유리하다.
-- 조회를 한 후 특정 칼럼의 값을 응용프로그램에서 사용하려고 할 때
--      위치(index)로 칼럼 값을 추출했을 때 정확히 원하는 위치값을 보장하여
--      오류를 줄일 수 있다.
...
FROM tbl_student;
-- 결과는 똑같지만 *를 지양하는 것이 속도면에서도 유리하고
-- 배열처럼 위치값으로 불러와야 하는 경우가 있는데 성능을 떠나 오류를 줄이고 정확도를 높이는 목적도 있다

-- 학생 테이블에서 이름이 홍길동이고 주소가 서울특별시인 데이터만 보고 싶다.
-- 다중조건 조회
-- AND 조건 : 두 조건이 모두 true인 리스트만 보여라 (교집합)
SELECT * FROM tbl_student
WHERE st_name = '홍길동' ANd st_addr = '서울특별시';

-- 학생 테이블에서 이름이 홍길동이거나 이몽룡인 데이터들을 보고싶다.
-- OR 조건 : 여러 조건중에 한 가지라도 true인 리스트를 보여라 (합집합)
SELECT * FROM tbl_student
WHERE st_name = '홍길동' or st_name = '이몽룡';
-- 이름이 홍길동이고 이몽룡인 데이터를 리스트로 나타내준다.

-- 학생 테이블에서 이름이 홍길동이거나 주소가 서울특별시인 데이터들을 보고 싶다.
SELECT * FROM tbl_student
WHERE st_name = '홍길동' or st_addr = '서울특별시';

-- 학생 테이블에서 이름이 이몽룡이거나 주소가 남원시인 데이터를 보고 싶다.
SELECT * FROM tbl_student
WHERE st_name = '이몽룡' or st_addr = '남원시';

-- 칼럼값들을 서로 연결해서 한 개의 문자열처럼 리스트를 보는 방법
...
--       어떤 데이터는 '서울시'라고 했다.
-- 이럴 경우 서울특별시라고 조회를 하면 서울시인 데이터는 보이지 않고 
-- 서울시 라고 조회를 하면 서울특별시 데이터는 보이지 않게 된다.
-- 이럴때 '서울'이라는 문자열이 포함된 데이터만 보고 싶을 때
...
-- 학생 테이블의 주소 칼럼에 저장된 문자열이 '서울'이라는 문자열로 시작되는 데이터들을 보여라
SELECT * FROM tbl_student
...
-- LIKE 연산자 키워드
--      SELECT 조회명령이 실행될 때 데이터가 많으면 매우 속도가 느려진다.
...
SELECT * FROM tbl_student
WHERE st_tel >= '010-111' AND st_tel <= '010-333' ;

-- 학생데이터를 조회하고 싶은데
-- 이름은 알지 못하고, 학번이 3번부터 6번 사이에 있었던 것 같다.
-- 이럴 때 학번 데이터는 문자열이지만 저장된 구조가 5자리로 일정하므로
-- 비교 연산자를 사용하여 데이터를 조회해 볼 수 있다.
SELECT * FROM tbl_student
WHERE st_num >= 'A0003' AND st_num <= 'A0006' ;

-- 어떤 범위내에 있는 데이터 리스트를 보고자 할 때
-- 학번이 A0003부터 A0006까지 범위의 데이터를 보고자 할 때
...
-- table에 데이터가 얼마가 저장되어있든 상관없이 
-- 출력되는 리스트는 1개이거나 없다.
-- 절대 2개이상 출력되지 않는다.
-- PK는 절대 중복데이터가 없다.
-- PK는 절대 비어있으면(NULL값이면) 안 된다. : NULL 값이면 안 된다
-- st_num가 PK로 되어 있는데 생략한 채로 데이터를 추가하려고 하면
-- 오류가 발생한다
INSERT INTO tbl_student(st_name, st_tel)
VALUES('김정은','010-999-9999'); -- 이 값은 추가될 수 없다.

-- DBMS의 임시저장소에 임시로 저장된 데이터를 storage에 저장하는 명령
-- DCL 명령, TCL(Transaction Control Lang)이라고도 함
COMMIT ;
