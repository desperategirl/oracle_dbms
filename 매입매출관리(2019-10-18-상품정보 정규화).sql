...
-- 상품정보 테이블에서 판매가격의 원 단위를 제거하고 0으로 세팅
...
-- 2. 매입총장의 상품코드 칼럼을 업데이터
--     서브쿼리를 사용해서
--   UPDATE 실행에서 유의사항
--          UPDATE를 수행하는 SUBQ의 SELECT Projdction에는 
--          칼럼을 1개만 사용해야 한다는 규칙이 있다.
--          SUBQ에서 나타나는 레코드수도 반드시 1개만 나타나야 한다.

-- 매입매출 테이블 리스트를 나열하고
-- 각 요소의 상품이름을 SUQ로 전달
-- SUQ에서는 상품테이블로부터 상품이름을 조회하여
-- 일치하는 레코드가 1개 나타나면
-- 해당레코드의 상품코드 칼럼의 값을
-- 매입매출칼럼의 상품코드 칼럼에 업데이트하라
...
실제 물리적 ㅔ이블이 저장되지 않은 사태이다
이땐느 롤백을 수행해서 CUD를 취소할 수 있다

단, ddl명령 (create, alter, drop)을 수행하면
자동 commit이 된다.
...
오라클에서는
commit 과 rollbakc도 시점을 지정해서
어디까지 rollback을 할 것인지 지정할수도 있다
*/
