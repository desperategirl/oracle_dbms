...
-- 두 테이블 간에 완전 참조무결성이 보장되는 경우
-- 이 조인이 표시하는 리스트를 카티션 곱이라고 표현한다.
SELECT * 
FROM tbl_books B, tbl_genre G
WHERE B.b_genre = G.g_code;

-- EQ 조인의 경우
-- 두 테이블이 완전 참조무결성 조건에 위배되는 경우, 결과가 신뢰성을 잃ㄴ,ㄴ디/

/*
참조 무결성 조건

-----------------------------------------
원본 Table(칼럼)    =       참조 Table(칼럼)
------------------------------------------
값이 있다          >>       반드시 있다
있을수도 있다       <<           있다
절대 있을 수 없다   <<           없다
...
FROM tbl_books B, tbl_genre G
WHERE B.b_genre = G.g_code ;
-- 그 결과 EQ JOIN으로 확인해 보니 새로 등록한 도서리스트가 누락되어 출력되었다.
-- 이제 출력결과는 신뢰성을 잃게 되었다.
...
-- 장르가 '장편소설'인 도서정보를 '장르소설'로 장르명칭을 변경하고 싶다.
