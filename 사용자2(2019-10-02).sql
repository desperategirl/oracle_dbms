...
-- WHITE SPACE라고 하며 실제로는 1개 이상의 space문자열이 저장되어 있다.
UPDATE tbl_address SET chain = ''
WHERE id =3;
SELECT * FROM tbl_address ;
...
WHERE DECODE(chain,'001','가족', 
        DECODE(chain,'002','친구', 
            DECODE(chain,'003','이웃'))) AS IS NULL;
...
-- 보기 위한 SQL이기도 하지만, 데이터를 검증하는 도구로 활용하기도 한다.
INSERT INTO tbl_address (id, namd, tel, chain)
VALUES(6, '장보고','010-777-7777','101');

-- 주소록 table에 저장된 모든 데이터를 아무런 조건없이 보여달라
...
INSERT INTO tbl_address(id, namd, tel)
...
-- 데이터를 조회할 때 특정 칼럼을 기준으로 정렬을 수행하여
-- 리스트를 보이기
...
-- 칼럼 이름 뒤에 DESC(DESCENDING)이 있으면 내림차순 정렬
-- 아무런 키워드가 없으면 ASC(ASCENDING)이 생략된 것이다. 써도 되고 안 써도 됨
--      오름차순(작은 -> 큰) 정렬
SELECT * FROM tbl_address
...
-- id 칼럼은 PK로 선언되어 있어서
-- 절대 중복값이 있을 수 없다.
-- 그럼에도 불구하고 칼럼들을 나열한 것은 의미없는 코드를 작성한 것이다.
SELECT * FROM tbl_address
