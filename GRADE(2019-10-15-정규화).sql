...
-- tlb_score에서 추출된 과목명을 저장할 TABLE 생성
...
-- 생성된 tbl_subject, tbl_score이 잘 연결되는지 테스트
SELECT *
FROM tbl_score SC, tbl_subject SB
    WHERE SC.s_subject = SB.sb_name;
    
-- 아래의 두 SQL문 결과 값이 같으면 정상적으로 수행완료
...
ALTER TABLE tbl_score ADD s_sbcode VARCHAR(4);
...
SET s_sbcode = 
(   SELECT sb_code FROM 
    tbl_subject SB WHERE SC.s_subject = SB.sb_name
);

/*
for(s:tbl_score) {
    where(s.sb_subject == tbl_subject.sb_name) {
