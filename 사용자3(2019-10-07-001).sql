...
-- tbl_books 테이블을 생성하고 데이터를 추가하다 보니 
...
MODIFY(b_price NUMBER(7)); -- 변경하겠다. 뭐를? 칼럼의 타입 또는 자릿수를...
...
--      데이터를 CRUD(추가 수정 삭제)를 실행할 때 문제가 발생할 수 있다.

-- 보통 테이블을 생성하거나 칼럼을 추가한 후로는 필요없더라도 다른 문제가 없으면
