...
-- JOIN이란?
-- 단순한 성적테이블 조회로는 구체적으로 학생의 이름이라든가
-- 다른 정보를 찾기가 매우 어렵다. 
-- 실제 학번과 연계된 학생테이블로부터 학생이름을 연결하고
...
-- 현재 학생 테이블에 학과코드 칼럼이 있음에도
-- SQL의 편의성을 고려하여 
-- 성적테이블에 학과코드를 추가하여 관리를 했다 : tbl_score2
-- 이런 상황이 되면 자칫 잘못하여 tbl_score2 테이블에 등록된 
-- 학과코드가 실제 학생의 학과코드가 아닌 값이 등록될 수 있다.
-- 이런 경우 데이터를 조회했을 때 실제 데이터와 차이가 있는 오류가 날 수 있다.

-- tbl_score 테이블과 tbl_student를 JOIN하고
-- 그 결과에서 학과코드를 기준으로 다시 tbl_dept와 JOIN을 수행하는 SQL을 연습
...
-->> WHERE에 의한 제한 LIST만 가지고 GROUP 실행
...
-- 생성된 2의 view JOIN
SELECT SC.학과, SC.이름, SC.학과코드, DP.학과이름,
SC.국어, SC.영어, SC.수학
...
DP.d_pro AS 담임교수,
        ...
        SC.s_eng AS 영어, 
        SC.s_math AS 수학,
        SC.s_kor + ST.s_eng + SV.s_math AS 총점,
        ((SC.s_kor + ST.s_eng + SV.s_math)/3,0) AS 평균,
        RANK() OVER ( ORDER BY (SC.s_kor + ST.s_eng + SV.s_math) DESC) AS 석차
FROM tbl_score SC
    LEFT JOIN tbl_student ST
        ON SC.s_num = ST.st_num
    LEFT JOIN tbl_dept DP
...
-- WHERE, HAVING 으로 어떤 조건을 부여할 때는
-- 길이가 가변적인 값으로 조회하기보다는
-- 고정된 길이의 값으로 조회하는 것이 결과를 더 확실하게 찾을 수 있다.
-- 학과이름으로 조건을 부여하는 것보다는
