...
-- ALTER TABLE을 사용해서 PK를 추가할 때는 가급적 이름을 지정해주는 것이 좋다.
...
-- 4. CHECK 제약조건
-- 데이터를 추가할 때 어떤 칼럼ㄹ에 저장되는 데이터를 제한하고자 할 때
ALTER TABLE tbl_test_std ADD CONSTRAINT CHECK ST_GRADE(BETWEEN 1 TO 4)
...
-- ALTER TABLE tbl_test_std ADD CONSTRAINT KEY_ST_NAME PRIMARY KEY(ST_NAME, ST_TEL);

-- ★ CONSTRAINT 이름을 지정하는 이유
--  이름이 지정되지 않으면 나중에 조건이 필요없어서 삭제를 하고자 할때
--  삭제가 어려워질 수도 있다.
